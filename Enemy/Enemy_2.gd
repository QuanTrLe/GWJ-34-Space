...
var intensity = Global.enemy_2_intensity
var duration = Global.enemy_2_duration
var type = "Random"

var speed = Global.enemy_2_speed
...
var attack_lag = Global.enemy_2_lag
...
func _ready():
	...
	$Laser_hitbox2/laser.visible = false
	$Laser_hitbox2.monitoring = false
	$Laser_hitbox2.monitorable = false
	
	$Laser_hitbox3/laser.visible = false
	$Laser_hitbox3.monitoring = false
	$Laser_hitbox3.monitorable = false
	
	$Laser_hitbox4/laser.visible = false
	$Laser_hitbox4.monitoring = false
	$Laser_hitbox4.monitorable = false
...
func _on_Player_detector2_area_entered(area):
	if !$AnimationPlayer.is_playing():
		area.get_parent().being_sucked()
		$Attack_lag.start(attack_lag)


func _on_Player_detector2_area_exited(_area):
	$Attack_lag.stop()


func _on_Laser_hitbox2_area_entered(area):
	if area.get_parent().is_in_group("Player"):
		var _unused_value = get_tree().reload_current_scene()
		

func _on_Player_detector3_area_entered(area):
	if !$AnimationPlayer.is_playing():
		area.get_parent().being_sucked()
		$Attack_lag.start(attack_lag)


func _on_Player_detector3_area_exited(_area):
	$Attack_lag.stop()


func _on_Laser_hitbox3_area_entered(area):
	if area.get_parent().is_in_group("Player"):
		var _unused_value = get_tree().reload_current_scene()
		

func _on_Player_detector4_area_entered(area):
	if !$AnimationPlayer.is_playing():
		area.get_parent().being_sucked()
		$Attack_lag.start(attack_lag)


func _on_Player_detector4_area_exited(_area):
	$Attack_lag.stop()


func _on_Laser_hitbox4_area_entered(area):
	if area.get_parent().is_in_group("Player"):
		var _unused_value = get_tree().reload_current_scene()
